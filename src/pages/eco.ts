...
function parse_eco_log(str: string): Result<EcoLog> {
  try {
    ...
    if (!("data" in json) || typeof json.data != "object") {
      throw new TypeError(
        "Invalid log: Logs must include a data object even if empty",
      );
    }
    const data_table_display =
...
export class Eco extends BasePage {
  ...
  protected async _on_mounted() {
    ...
    if (!this.dialog_element) {
      this.dialog_element = document.getElementById(
        "data-dialog",
      ) as HTMLDialogElement;
    }
  }

  protected async _on_full_data_display_request(
    this: { log: EcoLog },
    _ev: PointerEvent,
    context: { log: EcoLog & { $index: number } },
    _clicked_elem: HTMLElement,
    _ev_name: string,
    pui: { $parent: { $model: Eco } },
  ) {
    ...
    for (const key of Object.keys(log.data)) {
      ...
      if (typeof val != "object") {
        ...
        extra.push({
          ...
          template: `<li>\${key}: <pre><code>\${val}</code></pre></li>`,
        });
      }
    }
